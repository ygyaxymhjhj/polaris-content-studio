...
export const ASSET_SPECS: Record<Platform, { count: number; brief: string; minLength: number; notes: string[] }> = {
  ...
  facebook: { count: 2, minLength: 350, notes: ["visualBrief", "imageText", "firstComment"], brief: "Two independently publishable Facebook posts: A news/explainer angle, B a reader-question angle. Each content must open with a hook: one short first line that makes the story clear at a glance and carries the most striking sourced figure when the article has one — never force a figure in and never invent one. Then context, 2-4 concrete sourced details, a cautious takeaway and a natural question or CTA. Use 4-7 readable paragraphs, optional short bullets and 0-3 relevant hashtags. Target 150-250 English/Vietnamese words or 250-450 Chinese characters when source supports it. Do not output an outline, a teaser only, or instructions such as 'explain why it matters'. Include the real destination URL if provided; otherwise omit the link. meta: visualBrief (production direction, not evidence), imageText (short factual on-image headline), firstComment (optional publishable follow-up). Do NOT put visual directions into content." },
